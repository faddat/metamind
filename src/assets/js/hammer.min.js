...
(function(global, factory) {
	...
	function Panzoom(elem, options) {
		...
		this.isSVG = rsvg.test(this.namespaceURI) && this.nodeName.toLowerCase() !== 'svg';

		this.$set.map(function(v) {
			this.isSVG = rsvg.test(this.namespaceURI) && this.nodeName.toLowerCase() !== 'svg';
			return this;
		});
		...
		while (i--) {
			this.$set[i]._transform = !this.$set[i].isSVG && $.cssProps.transform.replace(rupper, '-$1').toLowerCase();
		}
	...
	Panzoom.prototype = {
		...
		setTransform: function(transform) {
			...
			var i = $set.length;
			while(i--) {
				$[$set[i].isSVG ? 'attr' : 'style']($set[i], 'transform', transform);
			}
		},
		...
		getTransform: function(transform) {
			...
			if (transform) {
				...
				transform = $[transformElem.isSVG ? 'attr' : 'style'](transformElem, 'transform');
			}
		...
		_initStyle: function() {
			...
			while (i--) {
				$(this.$set[i]).css({
					...
					'transform-origin': this.$set[i].isSVG ? '0 0' : '50% 50%'
				});
			}
			...
			if (!this.options.disablePan) {
				elemstyles.cursor = this.options.cursor;
			}
		...
		_buildTransition: function() {
			if (this._transform) {
				...
				var i = this.$set.length;
				while (i--) {
					this.$set[i]._transition = this.$set[i]._transform + ' ' + options.duration + 'ms ' + options.easing;
				}
			}
		},
